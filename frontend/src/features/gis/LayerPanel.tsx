...
export default function LayerPanel({ selectedLayer, onLayerChange, selectedTime, onTimeChange }: LayerPanelProps) {
  ...
  const getLayerType = (layerId: string): 'lst' | 'ndvi' | 'heatIndex' | null => {
    const layerMap: Record<string, 'lst' | 'ndvi' | 'heatIndex'> = {
      'lst': 'lst',
      'ndvi': 'ndvi',
      'heat-index': 'heatIndex'
    }
    return layerMap[layerId] || null
  }

  const isSelectedLayer = (layerId: string): boolean => {
    return selectedLayer === getLayerType(layerId)
  }
                ...
                <div className="pl-2 space-y-1">
                  {['lst', 'ndvi', 'heat-index'].map(layerId => {
                    ...
                    return (
                      <div 
                        key={layerId} 
                        className={`p-2 bg-white/5 rounded-lg cursor-pointer border-2 transition-colors ${
                          isSelectedLayer(layerId) ? 'border-primary' : 'border-transparent'
                        }`}
                        onClick={() => handleLayerSelect(layerId)}
                      >
